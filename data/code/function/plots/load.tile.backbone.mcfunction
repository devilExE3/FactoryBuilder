execute if data storage temp this{id:"conveyor"} run function code:blocks/backbone/conveyor
execute if data storage temp this{id:"chute"} run function code:blocks/backbone/chute
execute if data storage temp this{id:"elevator"} run function code:blocks/backbone/elevator
execute if data storage temp this{id:"sell"} run function code:blocks/backbone/sell

execute if data storage temp this{id:"cutter"} run function code:blocks/backbone/cutter
execute if data storage temp this{id:"furnace"} run function code:blocks/backbone/furnace
execute if data storage temp this{id:"crafter_2"} run function code:blocks/backbone/crafter_2
execute if data storage temp this{id:"crafter_3"} run function code:blocks/backbone/crafter_3
execute if data storage temp this{id:"washer"} run function code:blocks/backbone/washer
execute if data storage temp this{id:"flashbaker"} run function code:blocks/backbone/flashbaker
execute if data storage temp this{id:"sonic_zapper"} run function code:blocks/backbone/sonic_zapper
execute if data storage temp this{id:"crusher"} run function code:blocks/backbone/crusher
execute if data storage temp this{id:"enchanter"} run function code:blocks/backbone/enchanter
execute if data storage temp this{id:"limiter"} run function code:blocks/backbone/limiter
execute if data storage temp this{id:"splitter"} run function code:blocks/backbone/splitter
execute if data storage temp this{id:"air_conveyor"} run function code:blocks/backbone/air_conveyor
execute if data storage temp this{id:"air_sell"} run function code:blocks/backbone/air_sell
execute if data storage temp this{id:"explosive_furnace"} run function code:blocks/backbone/explosive_furnace
execute if data storage temp this{id:"crafter_5"} run function code:blocks/backbone/crafter_5

function code:plots/load.tile.backbone.generator